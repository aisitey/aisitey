...
export default function ArchitecturePage() {
  ...
  const [authRules, setAuthRules] = useState<string[]>([
    "Users authenticate through the configured authentication provider.",
  ...
  const [apiBoundaries, setApiBoundaries] = useState<string[]>([
    "Authentication and authorization checks happen in the route handler or shared middleware.",
  ...
  const [dataIntegrityRules, setDataIntegrityRules] = useState<string[]>([
    ...
    "Every child entity belongs to a valid parent entity.",
    "",
    "Status values must come from the defined status model.",
    "Deleting or modifying a resource must not bypass ownership checks.",
  ]);
  ...
  const [invariants, setInvariants] = useState<string[]>([
    ...
    "External input is validated before it reaches business logic.",
    "Business rules are not duplicated across unrelated components or routes.",
    "The status model defined in this file is the single source of truth.",
    "The architecture must not introduce additional infrastructure without a defined requirement.",
  ]);
  ...
  const parseMarkdown = (content: string) => {
    ...
    const parseResponsibilitySection = (
      heading: string,
    ) => {
      return cleanLines(getSection(heading))
        .filter(
          (line) =>
            !line.startsWith("UI components") &&
            !line.startsWith("Business logic") &&
            !line.startsWith("The architecture"),
        )
        .map(stripBullet);
    };
    ...
    const lifecycleHeading = lifecycleSection
      .map((line) => line.trim())
    ...
    if (lifecycleHeading) {
      const cleaned = lifecycleHeading
        .replace(/^Describe.*$/i, "")
        .trim();

      if (cleaned) {
        setLifecycleEntity(cleaned);
      }
    }
...
| --- | --- | --- |
${stackRows || "| Not specified | Not specified | Not specified |"}
...
Describe the separation between presentation, application logic, and persistence. Keep each layer's responsibility explicit so the agent knows where new code belongs.
...
${
  clean(applicationRule) ||
  "Business logic should not be duplicated across multiple routes or components."
}
...
${
  clean(fileStorage) ||
  "File/blob storage is not required yet. The storage strategy must be defined here before implementation."
}
...
List every core entity. For each one: what it represents, what fields it holds, and what it's related to.
...
## ${
      clean(lifecycleEntity) || "[Primary Entity]"
    } Lifecycle / Core Flow

Describe the controlled lifecycle of the main object moving through the system, step by step, if one exists.
...
This is the single source of truth for every status/enum value in the system.
...
Short, absolute rules that hold everywhere in the system, regardless of feature.
...
Any change that affects one of the following must be reflected in this file before implementation, when it affects multiple system boundaries:
...
Record why each major technology or structural choice was made — future sessions (human or AI) should not have to re-derive the reasoning.
      ...
      <div className="mx-auto max-w-4xl px-6 py-12">
        ...
        <div className="mt-10 space-y-8">
          ...
          <section className="rounded-2xl border border-default bg-surface p-6">
            ...
            <div className="mt-6 space-y-5">
              ...
              <ListSection
                ...
                placeholder="User creates the project"
                onAdd={() =>
                  addListItem(
                    lifecycleSteps,
                    setLifecycleSteps,
          ...
          <section className="rounded-2xl border border-default bg-surface p-6">
            ...
            <div className="mt-8 border-t border-default pt-6">
              <ListSection
                ...
                placeholder="Can a completed entity be reopened?"
                onAdd={() =>
                  addListItem(
                    statusOpenQuestions,
          ...
          <ListSection
            ...
            placeholder="Every resource belongs to an authenticated user."
            onAdd={() =>
              addListItem(
                dataIntegrityRules,
          ...
          <ListSection
            ...
            items={invariants}
            placeholder="Client components never access the database directly."
            onAdd={() =>
              addListItem(
                invariants,
                setInvariants,
          ...
          <ListSection
            ...
            placeholder="Database model changes"
            onAdd={() =>
              addListItem(
                architectureChangeRules,
